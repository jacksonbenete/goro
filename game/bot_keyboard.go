...
func registerLuaKeyboardAPI(state *lua.LState, api *lua.LTable, ctx client.Context) {
	...
	state.SetFuncs(keyboard, map[string]lua.LGFunction{
		"is_down": func(L *lua.LState) int {
			...
			L.Push(lua.LBool(ok && ctx.Input != nil && ctx.Input.KeyCodeDown(code)))
			return 1
		},
		"was_pressed": func(L *lua.LState) int {
			code, ok := input.KeyCodeFromName(L.CheckString(1))
			L.Push(lua.LBool(ok && ctx.Input != nil && ctx.Input.KeyCodeJustPressed(code)))
			return 1
		},
		"was_released": func(L *lua.LState) int {
			code, ok := input.KeyCodeFromName(L.CheckString(1))
			L.Push(lua.LBool(ok && ctx.Input != nil && ctx.Input.KeyCodeJustReleased(code)))
			return 1
		},
		"consume_press": func(L *lua.LState) int {
			code, ok := input.KeyCodeFromName(L.CheckString(1))
			L.Push(lua.LBool(ok && ctx.Input != nil && ctx.Input.ConsumeKeyCodePress(code)))
			return 1
		},
